...
//	float vpr = 4,vir = 3.5,vdr = 0.8;	//右轮PID
  ...
  //外设初始化
	...
	pid_init(&left_pid,vpl,vdl,vil,(float)0.99-DEADBAND,DEADBAND-(float)0.99);
	pid_init(&right_pid,vpr,vdr,vir,(float)0.99-DEADBAND,DEADBAND-(float)0.99);
	...
	UARTStringPut(UART7_BASE,str,4);
  while(1){
