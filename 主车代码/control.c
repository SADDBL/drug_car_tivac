...
//			destination_F = 1;
		...
		//返回起点
		case 15:{
			flag = point_to_point(3*CORRIDOR_LENGTH_1+(float)2.5*CORRIDOR_WIDTH);
			if(flag==OK){
				GREEN_ON;
				det_F = 0;
				car_reset();
...
/***** 双车通讯函数 *****/
//通讯规范：
//开头：a：传输车辆状态		b：传输数字识别结果
//结束：0xff

//车辆状态
//主车：1：到达目的地	2：开始返回	3：不再堵塞道路
//从车：1：出发	2：到达暂停点
...
void UARTStringPut(uint32_t uBase,unsigned char *string,uint8_t len){
	uint8_t i;
	for(i = 0;i<len;i++){
		UARTCharPutNonBlocking(UART7_BASE,string[i]);
	}
	//结束标志
	UARTCharPutNonBlocking(UART7_BASE,0xff);
}
